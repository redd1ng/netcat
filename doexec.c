    ...
    )
{
    ...
    if (!Result) {
        holler("Failed to create shell stdout pipe, error = %s",
			_itoa(GetLastError(), smbuff, 10), NULL, NULL, NULL, NULL, NULL);
        goto Failure;
    }
    ...
    )
{
    ...
    swprintf(lpszClientPath, MAX_PATH, L"%hs", pr00gie);
	if (CreateProcess(NULL, lpszClientPath, NULL, NULL, TRUE, 0, NULL, NULL,
