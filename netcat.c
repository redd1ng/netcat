...
#else
  if (_strcmpi (poop->name, hp->h_name) != 0) {	/* normal */
#endif
...
/* grab a socket; set opts */
  ...
  if (nnetfd == 0)		/* might *be* zero if stdin was closed! */
    nnetfd = _dup (nnetfd);	/* so fix it.  Leave the old 0 hanging. */
#ifdef WIN32
...
/* if you don't have all this FD_* macro hair in sys/types.h, you'll have to
  ...
  if (insaved) {
    ...
    if (Single)			/* if not scanning, this is a one-off first */
      ...
      _close (0);		/* so we won't need any more stdin */
    } /* Single */
  } /* insaved */
...
/* if no args given at all, get 'em from stdin and construct an argv. */
  if (argc == 1) {
	...
	insaved = _read (0, cp, BIGSIZ);	/* we're gonna fake fgets() here */
    if (insaved <= 0)
      bail ("wrong");
...
#ifdef GAPING_SECURITY_HOLE
  if (pr00gie) {
    _close (0);				/* won't need stdin */
    o_wfile = 0;			/* -o with -e is meaningless! */
    ofd = 0;
  }
#endif /* G_S_H */
  if (o_wfile) {
    ofd = _open (stage, O_WRONLY | O_CREAT | O_TRUNC, 0664);
